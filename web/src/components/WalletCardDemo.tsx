...
export const WalletCard = ({}) => {
  ...
  const chainsDat = [
    { chains: 'ajeioahg3iy73yq28rhbgajgoajeiohagoiheaoigh3pagawgih3a' },
    { chains: 'agoiejaoigjaigojawoighaghoagjheaiogjoeiagiawhgi' },
    { chains: 'agoiejaoigjaigojawoighaghiawhgi' },
    { chains: 'agoiejaoigjaigojawoighaghiawhgi' },
    { chains: 'agoiejaoigjaigojawoighaghiawhgi' },
  ]
  const defaultChain = 'ajeioahg3iy73yq28rhbgajgjopiehagioaghaigoih3a'
            ...
            <div className="absolute w-full min-h-[200px] text-white">
              <div className="inset-0 flex flex-col justify-between z-10 text-white">
                ...
                <div className="flex w-full mt-4 p-2 flex-col justify-between items-start text-xs">
                  <span className="flex items-center gap-2 px-2 py-1 rounded-full text-sm w-full min-w-0">
                    <span className="truncate">ChainId: {chain.chains}</span>
                    <Copy
                      size={14}
                      className="cursor-pointer text-gray-500 hover:text-gray-700 flex-shrink-0"
                      onClick={() => handleCopy(chain.chains)}
                    />
                  </span>
                  <span className="flex items-center gap-2 px-2 py-1 rounded-full text-sm w-full min-w-0">
                    <span className="truncate">Account: {chain.chains}</span>
                    <Copy
                      size={14}
                      className="cursor-pointer text-gray-500 hover:text-gray-700 flex-shrink-0"
                      onClick={() => handleCopy(chain.chains)}
              ...
              {defaultChain === chain.chains ? (
                ...
                <button
                  ...
                  className="text-black text-xs absolute top-1 right-0.5 border px-3 py-0.5 rounded-3xl flex justify-center items-center gap-1 bg-white/90 z-20"
                >
                  <RefreshCw width={15} />
                  Set Default
                </button>
